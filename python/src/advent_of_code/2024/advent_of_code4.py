...
def advent_of_code4a(test_input: str | list, pattern="XMAS") -> int:
    ...
    puzzle_matrix = Matrix(nrow=nrows, ncol=ncols, data=None)
    ...
    for row in range(0, nrows):
        row_count += len(
            re.findall(pattern=pattern, string="".join(puzzle_matrix[row]))
        )
        row_count += len(
            re.findall(pattern=reverse_pattern, string="".join(puzzle_matrix[row]))
        )

    for col in range(0, ncols):
        col_count += len(
            re.findall(pattern=pattern, string="".join(puzzle_matrix.get_column(col)))
        )
        col_count += len(
            re.findall(
                pattern=reverse_pattern, string="".join(puzzle_matrix.get_column(col))
            )
        )
    ...
    for row in range(0, nrows):
        for col in range(0, ncols):
            if row + pattern_len - 1 < nrows and col + pattern_len - 1 < ncols:
                sub_matrix = Matrix(pattern_len, pattern_len, data=None, fill="_")
                ...
                diag_count += len(
                    re.findall(
                        pattern=pattern, string="".join(sub_matrix.get_diagonal())
                    )
                )
                diag_count += len(
                    re.findall(
                        pattern=pattern,
                        string="".join(sub_matrix.get_reverse_diagonal()),
                    )
                )
                diag_count += len(
                    re.findall(
                        pattern=reverse_pattern,
                        string="".join(sub_matrix.get_diagonal()),
                    )
                )
                diag_count += len(
                    re.findall(
...
def advent_of_code4b(test_input: str | list, cross_pattern="MAS") -> int:
    ...
    for row in range(0, nrows):
        for col in range(0, ncols):
            if row + pattern_len - 1 in range(
                0, nrows
            ) and col + pattern_len - 1 in range(0, ncols):
                sub_matrix = Matrix(pattern_len, pattern_len, data=None, fill="_")
